...
describe('NorthPaw Review Prompt Flow & Eligibility Engine', () => {
  ...
  it('Test 9: Invokes native review flow when available, or opens store URL fallback when native API fails', async () => {
    // Sub-test A: Native API Available
    mockStoreReview.isAvailableAsync.mockResolvedValue(true);
    mockStoreReview.requestReview.mockResolvedValue(undefined);
    ...
    // Sub-test B: Native API Unavailable -> Store Fallback URL
