...
async function checkReviewEligibility(params) {
  const { onboardingDone, isCriticalFlow, todayOverride } = params || {};
...
async function handleLeaveAReview() {
  ...
  await trackEvent('review_native_flow_attempted', {
    platform: Platform.OS,
    app_version: appVersion,
    unique_usage_days: data.uniqueUsageDays.length,
  });

  // 3. Attempt native platform review or fallback URL
  try {
    ...
    console.warn('[ReviewPrompt] Native review error, fallback to store page:', e);
    await openStoreFallback();
  }
}
