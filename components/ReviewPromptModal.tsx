...
export function ReviewPromptModal({ visible, onClose }: ReviewPromptModalProps) {
  ...
  return (
    <Modal
      ...
      transparent
      animationType="fade"
      onRequestClose={rated ? onNotNow : onMaybeLater}
    >
      <View style={styles.overlay}>
        <Pressable style={StyleSheet.absoluteFill} onPress={rated ? onNotNow : onMaybeLater} />
        ...
        >
          ...
          <Text style={[styles.body, { color: palette.textSecondary, marginBottom: rated ? 12 : 16 }]}>
            {rated 
              ? `You rated NorthPaw ${selectedStars} Stars! Would you like to write a review on the App Store to help other dog owners?`
              : 'Tap a quick star rating below:'}
          </Text>

          {/* Star Display / Selection */}
          <View style={styles.starRow}>
            {[1, 2, 3, 4, 5].map((star) => (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.starBtn,
                  { transform: [{ scale: pressed && !rated ? 1.25 : 1 }] }
                ]}
          ...
          {!rated ? (
            ...
            <>
              {/* Secondary Options Step (Post-Rating) */}
              {selectedStars >= 4 ? (
                <Pressable
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.secondaryBtn,
                  {
                    backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)',
                    opacity: pressed ? 0.85 : 1,
                    marginTop: selectedStars >= 4 ? 4 : 14,
                  },
                ]}
              >
                <Text style={[styles.secondaryBtnText, { color: palette.textSecondary }]}>Not Now</Text>
              </Pressable>
            </>
          )}
        </AnimatedReanimated.View>
